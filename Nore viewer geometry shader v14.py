...
VR_WORLD_SCALE = 20000.0  # 物理空间 1 米 = 虚拟空间 20000 米
                          # V14 调整: 适应短线缆 VR 设备，走 0.3 米就能穿过模型半径
# ▌近距离裁剪半径，世界坐标单位。
# ▌0.0 = 关闭（不裁剪任何单元）。
...
def main():
    ...
    while not glfw.window_should_close(win):
        ...
        if vr_mode and vr_system is not None and vr_fbo_l is not None:
            try:
                ...
                if hmd_pose.bPoseIsValid:
                    ...
                    # ── 关键：构造让"模型固定在世界原点"的视图矩阵 ──
                    # 模型放置在世界原点附近，用户初始位置在模型前方 VR_USER_OFFSET 米处。
                    # 由于油藏坐标尺度大（约9000米），需要把头显的小位移放大。
                    #
                    # 数学推导：
                    #   设用户初始世界位置 = (0, 0, +VR_USER_OFFSET)
                    #   设头显追踪位置    = hmd_mat[0:3, 3] (米)
                    #   则用户当前世界位置 = (0, 0, VR_USER_OFFSET) + hmd_mat * VR_WORLD_SCALE
                    #
                    # 视图矩阵 = inverse(camera-to-world)
                    # camera-to-world: 头显的旋转 + 用户当前世界位置

                    # 提取头显的旋转部分（3×3 方向矩阵）
                    hmd_rot = np.eye(4, dtype=np.float32)
                    hmd_rot[:3, :3] = hmd_mat[:3, :3]

                    # 头显在物理空间的位置（米），放大到虚拟空间
                    hmd_pos_phys = hmd_mat[:3, 3]
                    hmd_pos_world = hmd_pos_phys * VR_WORLD_SCALE

                    # 用户在世界坐标的最终位置 = 初始偏移 + 头显追踪位置
                    ...
                    # camera-to-world 矩阵：先旋转，再平移到用户位置
                    cam_to_world = hmd_rot.copy()
                    ...
                    # 调试：每60帧打印一次位置信息
                    if frame_count % 60 == 0:
                        dist_to_origin = float(np.linalg.norm(user_world_pos))
                        print(f"[VR追踪] 头显物理位置: ({hmd_pos_phys[0]:.2f}, {hmd_pos_phys[1]:.2f}, {hmd_pos_phys[2]:.2f}) m")
                        print(f"[VR追踪] 用户虚拟位置: ({user_world_pos[0]:.0f}, {user_world_pos[1]:.0f}, {user_world_pos[2]:.0f})")
                        print(f"[VR追踪] 到模型中心距离: {dist_to_origin:.0f}  (模型半径≈6000, Shell半径={shell_radius:.0f})")
                else:
                    # 头显姿态无效：使用安全的回退值
                    V_vr = mat_lookat(
        ...
        glfw.set_window_title(win,
            f"FPS:{fps_value:.1f}  Norne v10 [{mode_str}]  |  X:{slice_x*100:.0f}%  Z:{slice_z*100:.0f}%  "
            f"VExag:{vexag:.1f}x  "
            f"Faults:{'ON' if show_faults else 'OFF'}  "
            f"Wells:{'ON' if show_wells else 'OFF'}  "
            f"Prox:{proximity_radius:.0f}  Shell:{shell_radius:.0f}  " + ("[VR: 走近模型→外观, 走入模型→内壁]" if vr_mode else f"[I]={'退出内部' if inside_mode else '进入内部'}  [V]=切换VR"))
        ...
        if now - fps_last_time >= 0.5:
            fps_value = fps_frame_count / (now - fps_last_time)
            fps_frame_count = 0
            fps_last_time = now
