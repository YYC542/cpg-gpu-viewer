...
// ============================================================
// V20最终版: 顶点推远函数 (Vertex Repulsion)
// ============================================================
// 导师要求: "靠近观察者的单元不应简单消失, 而应在视觉上
//           封闭油藏内部的空洞部分, 使油藏始终为封闭实体。"
//
// 原理: 不隐藏任何单元。如果顶点离相机太近(< uShellRadius),
//       就把它"推"到以相机为中心的球面上。
//       效果: 近端单元被膨胀成包围观察者的封闭壳面,
//             远端单元不受影响, 保持原始几何。
//       没有任何东西消失, 没有尖刺, 没有空洞。
vec3 repel(vec3 p) {
    vec3 dir = p - uCamPos;
    float d = length(dir);
    if (d < uShellRadius && d > 0.01) {
        // 平滑过渡：越近推得越多，边界处几乎不推
        float t = d / uShellRadius;          // 0=相机位置, 1=壳边界
        float newDist = mix(uShellRadius, d, t*t*t);  // 三次平滑
        return uCamPos + normalize(dir) * newDist;
    }
    return p;
}
...
// ▌输出: 一个 triangle_strip 图元 (4个顶点 → 2个三角形 → 1个四边形面)
    ...
    vec3 ae=repel(vex(a)), be=repel(vex(b)), ce=repel(vex(c)), de=repel(vex(d));
...
// ▌对每个输入点 (= 每个网格单元) 执行一次。
    ...
    // V20 最终版: 顶点推远 + 双面渲染
    // ══════════════════════════════════════════════════════════
    // 导师要求: "单元不应消失, 应封闭内部空洞, 始终为封闭实体。"
    //
    // 方案: repel() 函数把太近的顶点推到球面上（消除尖刺），
    //       近端单元同时渲染外+内表面（封闭空洞）。
    //       没有任何单元被隐藏。
    //
    //   远端 (dist > shellR): 仅外表面（正常渲染）
    //   近端 (dist <= shellR): 外表面 + 内表面（封闭壁面）
    //       顶点经 repel() 推远后不会产生尖刺
...
def main():
    ...
    global shell_radius
    shell_radius = sz * 0.5
    print(f"[Shell] shell_radius={shell_radius:.0f} (repel半径, sz={sz:.0f})")
    ...
    while not glfw.window_should_close(win):
        ...
        # B/N键: 调节Proximity Culling半径 (仅桌面模式, VR模式下自动)
        if not vr_mode:
            prox_step = sz * 0.005
        ...
        if vr_mode and vr_system is not None and vr_fbo_l is not None:
            try:
                ...
                # V20最终版: VR 三层渲染自动参数
                # ════════════════════════════════════════════════════════════
                # 导师要求: 近端单元封闭空洞，不是简单消失。
                # 三层: 极近(隐藏) + 边界(内表面封闭) + 远端(正常外表面)
                #
                # proximity_radius: 极近隐藏半径（消除尖刺）
                # shell_radius: 边界层外边界（封闭壁面）
                model_radius = sz * 0.5
                dist_to_center = float(np.linalg.norm(vr_cam_pos))
                # ════════════════════════════════════════════════════════════
                # V20最终版: 顶点推远参数
                # ════════════════════════════════════════════════════════════
                # repel() 把太近的顶点推到 shell_radius 球面上。
                # shell_radius 同时也是"渲染内表面"的判断距离。
                # 不使用 proximity（不隐藏任何单元）。
        ...
        glfw.set_window_title(win,
            f"Norne v20 [{mode_str}]  |  X:{slice_x*100:.0f}%  Z:{slice_z*100:.0f}%  "
            f"VExag:{vexag:.1f}x  "
