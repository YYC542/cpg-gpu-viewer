...
# ════════════════════════════════════════════════════════════════
# V17 (本版本) 相对 V16 的改动:
#   VR 模式下完全禁用所有显示控制键盘按键 (Q/A/E/D/Z/X/N/B/F/W)。
#   仅保留两个全局键:
#     - ESC : 退出程序
#     - V   : 在 VR / 桌面模式之间切换
#   配合 V14 已有的 I 键禁用，VR 模式下完全无法通过键盘修改任何
#   显示参数。这确保 VR 体验完全由头显控制，符合"严格 VR-only"模式。
#   桌面模式下所有键盘控制保持不变。
#
# V16 → V17 不变的部分:
#   - 启动时自动检测头显 (V16 引入)
#   - 模型静止、6DoF 头显追踪、球壳变形 (V14/V15)
# ════════════════════════════════════════════════════════════════
...
def main():
    ...
    while not glfw.window_should_close(win):
        ...
        # ════════════════════════════════════════════════════════════════
        # V17 修改：所有显示控制键盘按键 (Q/A/E/D/Z/X/N/B/F/W) 仅在
        # 桌面模式下生效。VR 模式下完全禁用键盘控制，体验由头显独占。
        # ════════════════════════════════════════════════════════════════
        if not vr_mode:
            # --- 连续按键 (持续响应) ---
            step = 0.003
            ...
            # V17: VR 模式下，仍要更新 prev_f / prev_w 状态以避免切回桌面时误触
            prev_f = glfw.get_key(win, glfw.KEY_F)
            prev_w = glfw.get_key(win, glfw.KEY_W)
        ...
        if vr_mode and vr_system is not None and vr_fbo_l is not None:
            try:
                ...
                if hmd_pose.bPoseIsValid:
                    ...
                    #     print(f"[VR] dist_to_origin={dist:.0f}, shell={shell_radius:.0f}")
                else:
                    # 头显姿态无效：使用安全的回退值
                    V_vr = mat_lookat(
