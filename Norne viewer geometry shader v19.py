...
VR_WORLD_SCALE = 5000.0   # 物理空间 1 米 = 虚拟空间 5000 米
# V19: 从 20000 降到 5000。原因: scale=20000 时转头引起的 5cm 弧形位移
#   被放大到 1000m 虚拟漂移(7度视角偏移)，导致模型看起来"跟着头晃"。
#   scale=5000 时同样的转头只产生 250m 虚拟漂移(1.8度)，几乎无感。
#   走进模型: 8000/5000 = 1.6 米 ≈ 2-3 步，符合"走几步进入模型"的需求。
...
def main():
    ...
    while not glfw.window_should_close(win):
        ...
        if vr_mode and vr_system is not None and vr_fbo_l is not None:
            try:
                ...
                if hmd_pose.bPoseIsValid:
                    ...
                    hmd_pos_world = hmd_pos_phys_rel * VR_WORLD_SCALE
        ...
        glfw.set_window_title(win,
            f"Norne v19 [{mode_str}]  |  X:{slice_x*100:.0f}%  Z:{slice_z*100:.0f}%  "
            f"VExag:{vexag:.1f}x  "
