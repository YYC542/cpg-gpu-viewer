# Author:Ach1n
# Data:2026/3/19 17:05
# Flag:I earned it.
# ============================================================================
# 文件名: Norne_viewer_geometry_shader__V8.py
...
# ▌  4) 三点光照模型 (代替 V7 的单光源)
# ▌  5) 双面法线处理 (abs() 消除黑面)
# ▌  6) 多边形偏移 (Polygon Offset) 减少 z-fighting 闪烁
# ▌  7) 背面剔除 (Back-face Culling) 提升渲染效率
# ▌
...
def main():
    ...
    while not glfw.window_should_close(win):
        ...
        # ▌将 MVP 矩阵传给着色器。
        # ▌GL_TRUE: 告诉 OpenGL 做转置 (因为 NumPy 是行主序, OpenGL 期望列主序)。
        ...
        # ▌将归一化的切片位置 [0,1] 映射回实际坐标范围:
        # ▌  slice_x = 0.6 → uSliceX = xmin + 0.6*(xmax-xmin)
        # ▌  即 X 坐标的 60% 位置。
        ...
        # ▌  glBindTexture(): 将角点纹理绑定到该单元
        # ▌  glUniform1i(uCorns, 0): 告诉着色器 uCorners 采样器使用纹理单元 0

        glActiveTexture(GL_TEXTURE1)
        glBindTexture(GL_TEXTURE_BUFFER, tbo_faults)
        glUniform1i(uFault, 1)
        # ▌将断层 TBO 绑定到纹理单元 1 (V8 新增)。

        glBindVertexArray(vao_res)
        # ▌绑定油藏网格的 VAO (包含深度属性配置)。

        glDrawArrays(GL_POINTS, 0, ncells)
        # ▌关键绘制调用! 画 ncells 个点。
        # ▌每个点触发一次几何着色器执行，在 GPU 端"膨胀"为一个六面体。
        # ▌这一条命令就渲染了约 11.3 万个完整的六面体!
        ...
        # ▌实时在标题栏显示当前参数值:
        # ▌  X 切面百分比, Z 切面百分比, 垂直夸大倍数, 断层/井轨迹开关状态。
