...
VR_USER_OFFSET = 8000.0    # 用户初始距模型中心的距离（米）
VR_SCALE       = 15000.0   # 统一缩放（所有轴相同）
# 走进模型: 8000/15000 = 0.53 米 ≈ 半步
# 转头残余: 颈部补偿后约1cm × 15000 = 150m, 在8000m处 = 1.07度
# ▌近距离裁剪半径，世界坐标单位。
# ▌0.0 = 关闭（不裁剪任何单元）。
...
// ▌6 外表面 + 6 内表面 = 12 面 × 4 顶点 = 48（仍在 GPU 上限内）。

in  float vDepth[];
out float fDepth;
out vec3  fNormal;
out float fFault;
out float fInterior;   // 0=外表面, 1=内表面（片段着色器据此高亮）
...
uniform vec3  uCamPos;          // 相机世界坐标（CPU端已把 z 乘以 uVExag，与 vex 同空间）
uniform float uProximityRadius; // V22: 保留但不再使用（不隐藏任何单元）
uniform float uShellRadius;     // V22: 气泡半径 = 封闭壳外边界
uniform float uTileScale;       // V22: 近端单元角度放大系数（填补立体角缝隙）
...
// ============================================================
// V22 方案A: 逐单元刚性气泡变换 (Per-cell Rigid Bubble)
// ============================================================
// 导师要求: 近端单元不应"消失", 而应被几何重组, 在视觉上封闭
//           内部空洞, 使油藏从任意距离/任意角度都是封闭连续实体。
//
// 旧 repel() 的致命缺陷: 逐"顶点"推远 + 硬截断。同一个面的 4 个角点
//   到相机的距离不同 -> 有的被推到球面、有的原地不动 -> 面被拉成
//   从球面延伸到远端的无穷长三角形 -> 满屏放射状尖刺。
//
// V22 改法: 一切以"单元中心"为单位。对中心算一次径向变换, 然后把
//   完全相同的仿射变换（平移 + 绕新中心的均匀缩放）施加到 8 个角点。
//   单元因此始终保持完整六面体, 面永远不会被撕裂 -> 根除尖刺。
//
// 三段式（dc = 单元中心到相机距离, 均在 vex 空间）:
//   远端  dc > shellR          : 原样输出（真实远端油藏）
//   过渡  midR < dc <= shellR   : 刚性径向推远（缓冲带, 连续过渡）
//   近端  dc <= midR            : 投影到 midR 球面 + 角度放大（铺满立体角）
//
// 连续性: 中心位置在 midR 与 shellR 两个边界都连续（无跳变）;
//         放大系数在 midR 处=1, 与过渡带平滑衔接。
//
// 说明（方案A的已知边界）: 角度放大能盖住大部分缝隙, 但若某个方向
//   "本来就没有活跃单元"（Norne 约 60% 单元非活跃）, 径向变换无法
//   凭空造出单元, 该方向仍可能残留小缝。完全零缝需方案B（细分/角度
//   重分配, 提高 max_vertices）。本版本先消除尖刺并尽量封闭。
vec3 bubble(vec3 vc, vec3 cc, float dc) {
    float shellR = max(uShellRadius, 1.0);
    float midR   = shellR * 0.5;

    if (dc > shellR) return vc;                 // 远端: 原样

    vec3  d3  = cc - uCamPos;
    vec3  dir = (dc > 1e-3) ? d3 / dc : vec3(0.0, 0.0, 1.0);

    float targetDist;
    float scale;
    if (dc > midR) {
        // 过渡带: 刚性径向推远; 外边界处不动, 内边界对齐 midR
        float t = (dc - midR) / max(shellR - midR, 1.0);   // 0..1
        targetDist = mix(midR, dc, t);
        scale = 1.0;
    } else {
        // 近端: 投影到 midR 球面 + 放大（越近放大越多, midR 处=1 不突变）
        targetDist = midR;
        scale = mix(uTileScale, 1.0, dc / midR);
    }

    vec3 nc = uCamPos + dir * targetDist;        // 新单元中心
    return nc + (vc - cc) * scale;               // 刚性: 同一中心 + 同一缩放
}

// 发射外表面（顶点已是最终 vex+bubble 坐标）
void emitFace(vec3 a, vec3 b, vec3 c, vec3 d, float depth, float fault) {
    vec3 n = normalize(cross(b - a, c - a));
    fNormal = n; fDepth = depth; fFault = fault; fInterior = 0.0;
    gl_Position = uMVP * vec4(a, 1.0); EmitVertex();
    gl_Position = uMVP * vec4(b, 1.0); EmitVertex();
    gl_Position = uMVP * vec4(c, 1.0); EmitVertex();
    gl_Position = uMVP * vec4(d, 1.0); EmitVertex();
    EndPrimitive();
}

// 发射内表面（翻转绕序 -> 从气泡内部可见 -> 封闭空洞）
void emitFaceInt(vec3 a, vec3 b, vec3 c, vec3 d, float depth, float fault) {
    vec3 n = normalize(cross(c - a, b - a));
    fNormal = n; fDepth = depth; fFault = fault; fInterior = 1.0;
    gl_Position = uMVP * vec4(a, 1.0); EmitVertex();
    gl_Position = uMVP * vec4(c, 1.0); EmitVertex();
    gl_Position = uMVP * vec4(b, 1.0); EmitVertex();
    gl_Position = uMVP * vec4(d, 1.0); EmitVertex();
...
void main() {
    ...
    // vex 空间角点 + 中心
    vec3 v0=vex(c0), v1=vex(c1), v2=vex(c2), v3=vex(c3);
    vec3 v4=vex(c4), v5=vex(c5), v6=vex(c6), v7=vex(c7);
    vec3 cc = vec3(cx, cy, cz * uVExag);
    float dc = distance(cc, uCamPos);

    // 逐单元刚性气泡变换（8 角点同一变换 -> 永不撕裂）
    vec3 b0=bubble(v0,cc,dc), b1=bubble(v1,cc,dc);
    vec3 b2=bubble(v2,cc,dc), b3=bubble(v3,cc,dc);
    vec3 b4=bubble(v4,cc,dc), b5=bubble(v5,cc,dc);
    vec3 b6=bubble(v6,cc,dc), b7=bubble(v7,cc,dc);

    float shellR = max(uShellRadius, 1.0);

    // 外表面（所有单元都画）
    emitFace(b0,b2,b1,b3, cz, fault);
    emitFace(b4,b5,b6,b7, cz, fault);
    emitFace(b0,b1,b4,b5, cz, fault);
    emitFace(b3,b2,b7,b6, cz, fault);
    emitFace(b2,b0,b6,b4, cz, fault);
    emitFace(b1,b3,b5,b7, cz, fault);

    // 内表面（仅气泡内单元: 从内部封闭空洞, 形成连续壳壁）
    if (dc <= shellR) {
        emitFaceInt(b0,b2,b1,b3, cz, fault);
        emitFaceInt(b4,b5,b6,b7, cz, fault);
        emitFaceInt(b0,b1,b4,b5, cz, fault);
        emitFaceInt(b3,b2,b7,b6, cz, fault);
        emitFaceInt(b2,b0,b6,b4, cz, fault);
        emitFaceInt(b1,b3,b5,b7, cz, fault);
    }
}
"""
...
// ▌片段着色器入口: 对每个像素 (fragment) 执行一次。
    ...
    // V20: 内表面视觉增强 — 让"进入模型内部"的效果一目了然
    if (fInterior > 0.5) {
        // 内表面: 增亮 + 加一层淡白色光晕，模拟"被岩层包围"的感觉
        FragColor.rgb = FragColor.rgb * 1.3 + vec3(0.08, 0.08, 0.12);
    }
}
"""
...
def main():
    ...
    global shell_radius
    shell_radius = sz * 0.15   # V22: 气泡半径（封闭壳外边界）
    print(f"[Shell] shell_radius={shell_radius:.0f}, tile_scale={tile_scale:.1f} (sz={sz:.0f})")
    ...
    print("  N / B      : Bubble gap-fill scale (tile_scale) up / down")
    print("  . / ,      : Bubble radius (shell_radius) up / down")
    ...
    while not glfw.window_should_close(win):
        ...
        # ════════════════════════════════════════════════════════════
        # V22: 气泡可视化实时调参（VR + 桌面均可用, 仅改渲染参数不动相机）
        #   N / B : 增大 / 减小 近端单元角度放大系数 tile_scale（填缝）
        #   . / , : 增大 / 减小 气泡半径 shell_radius（封闭壳大小）
        # ════════════════════════════════════════════════════════════
        if glfw.get_key(win, glfw.KEY_N) == glfw.PRESS:
            tile_scale = min(6.0, tile_scale + 0.02)
        if glfw.get_key(win, glfw.KEY_B) == glfw.PRESS:
            tile_scale = max(1.0, tile_scale - 0.02)
        shell_step = sz * 0.004
        if glfw.get_key(win, glfw.KEY_PERIOD) == glfw.PRESS:
            shell_radius = min(sz * 1.0, shell_radius + shell_step)
        if glfw.get_key(win, glfw.KEY_COMMA) == glfw.PRESS:
            shell_radius = max(sz * 0.02, shell_radius - shell_step)
            ...
            try:
                ...
                if hmd_pose.bPoseIsValid:
                    ...
                    if hmd_init_inv is None:
                        ...
                        print(f"[VR]   走 {VR_USER_OFFSET/VR_SCALE:.2f} 米 ≈ 走到模型中心")
                        print(f"[VR]   统一缩放: ×{VR_SCALE:.0f} + 颈部模型补偿")
                ...
                # V20最终版: VR 三层渲染自动参数
                # ════════════════════════════════════════════════════════════
                # V22: 气泡封闭参数（逐单元刚性变换，不隐藏任何单元）
                # ════════════════════════════════════════════════════════════
                # 导师要求: 近端单元不消失，而是被几何重组以封闭内部空洞。
                # bubble()（几何着色器内）按单元中心距离三段处理：
                #   远端原样 / 过渡刚性推远 / 近端投影到 shell_radius/2 球面+放大。
                # shell_radius 同时是"渲染内表面"的判断距离。
                # proximity_radius 保留为 0（V22 不再隐藏任何单元）。
                proximity_radius = 0.0    # V22: 不隐藏任何单元
                shell_radius = sz * 0.15  # V22: 气泡半径（可用 ,/. 实时微调）
        ...
        glfw.set_window_title(win,
            f"Norne v22 [{mode_str}]  |  X:{slice_x*100:.0f}%  Z:{slice_z*100:.0f}%  "
            f"VExag:{vexag:.1f}x  "
            f"Faults:{'ON' if show_faults else 'OFF'}  "
            f"Wells:{'ON' if show_wells else 'OFF'}  "
            f"Shell:{shell_radius:.0f}  Tile:{tile_scale:.1f}  " + ("[VR: 走入模型  N/B=填缝  ,/.=壳大小  R=重锚]" if vr_mode else f"[I]={'退出内部' if inside_mode else '进入内部'}  N/B=填缝 ,/.=壳大小 [V]=VR"))
