...
proximity_radius = 0
shell_radius = 3750.0  # V20: 初始值，main 中根据 sz 自动调整
...
// ▌对每个输入点 (= 每个网格单元) 执行一次。
    ...
    // V20 最终版: 三层渲染（晶洞效果）
    // ══════════════════════════════════════════════════════════
    // 导师要求: "靠近观察者的单元不应简单消失, 而应封闭内部空洞,
    //           使油藏始终表现为封闭、连续的实体。"
    //
    //   1. 极近 (dist < proxR): 不渲染（太近会被透视拉伸成尖刺）
    //   2. 边界 (proxR <= dist <= shellR): 渲染内表面（封住空洞壁面）
    //   3. 远端 (dist > shellR): 标准外表面
    //
    //   效果: 走进模型后像站在一个彩色晶洞里，四周被封闭的
    //         深度着色壁面包围，没有尖刺也没有黑色空洞。

    float proxR  = max(uProximityRadius, 0.0);
...
def main():
    ...
    global shell_radius
    shell_radius = sz * 0.25
    print(f"[Shell] 初始 shell_radius={shell_radius:.0f}, proximity=0 (VR中自动调整)")
    ...
    while not glfw.window_should_close(win):
        ...
        if vr_mode and vr_system is not None and vr_fbo_l is not None:
            try:
                ...
                if dist_to_center < model_radius:
                    # 在模型内部: 开启三层渲染
                    proximity_radius = sz * 0.08   # 极近隐藏（约 1200m）
                    shell_radius = sz * 0.25       # 边界层到 3750m
                else:
                    # 在模型外部: 正常渲染
