...
# ════════════════════════════════════════════════════════════════
# V20 (本版本) 修复:
#   ★ 修复 VR 模式下"走进模型时出现丑陋尖刺"的视觉问题 ★
#
#   根因1: 过渡区(1.0<r<1.5)同时渲染内外表面，近距离下外表面产生尖刺。
#   根因2: cellCenter.z 用了 vexag 但 uCamPos.z 没有, 坐标空间不一致。
#
#   修复: 彻底去掉过渡区，简化为两区域:
#         远端(dist>shellR) → 正常外表面
#         近端(dist<=shellR) → 仅内表面(原始几何,无球面投影)
#         同时修复 uCamPos.z 乘以 vexag 的坐标不一致问题。
#   ★ 修复"每次运行模型位置都不一样、模型出现在头顶"的 bug ★
#
#   根因: V16 直接把 SteamVR 追踪空间的绝对坐标 (含头部高度Y≈1.6米)
#         乘以 VR_WORLD_SCALE，导致用户被传送到模型上空。
...
proximity_radius = 0
shell_radius = 7500.0  # V20: repel 半径，main 中自动调整
tile_scale = 2.0  # V22: 近端单元角度放大系数（N/B 实时调节）
# V26: 自动进入参数
auto_enter = True          # 自动进入开关（True=靠近自动剥壳, 无需按键）
prox_max_factor = 0.12     # 走到中心时的剥壳半径上限 = sz * 此系数
model_radius_factor = 1.0  # 触发进入的模型半径 = 最大半跨 * 此系数
z_dir = 1.0                # V29-fix2: 前探方向(用户实测 -1 是后退进入, 改回 +1; 若仍反按 T 键)
# 原来固定 3000 太小了: 模型跨度约 15000m，只有极少数近端单元渲染内表面。
# 设为 sz*2 后: 只要用户在模型内部，所有单元都同时渲染外+内表面，
# 从内部看到完整封闭的洞穴壁，从外部看仍然是正常外观。

# ════════════════════════════════════════════════════════════════════
# V14 新增: VR 6DoF 配置常量
# ════════════════════════════════════════════════════════════════════
# 模型固定在世界原点 (0,0,0)，用户初始位置在模型前方 VR_USER_OFFSET 米处。
# VR_SCALE 控制头显的物理位移如何映射到虚拟世界:
#   Z轴(前后): 高 scale → 走2步就能进入模型
#   统一缩放 + 颈部模型补偿 → 转头时模型几乎不动
VR_USER_OFFSET = 9000.0    # V27: 起点在模型外侧(模型半径≈7642), 戴上头显先看到完整模型
# V28: 分轴缩放 — 解决"短线VR走不动" + "转头跟晃"两个问题。
VR_SCALE_Z     = 4000.0    # V29: 前后缩放(原8000太灵敏冲过头) -> 头前探~0.34m进入模型; Z不受颈部补偿干扰
VR_SCALE_XY    = 200.0     # V28: 左右/上下缩放小 -> 转头、上下看时模型几乎不跟晃(5cm×200=10m@9000m≈0.06°)
# V28 交互: 坐着/站着不动, 头向前探 -> 钻进模型(自动剥壳露出内部); 头收回/后仰 -> 退出。
#   适合 VR 线短、无法在房间走动的场景。左右转头看模型四周不会跟晃。
# 按 G 键随时一键回到模型中心; 按 R 键重新锚定朝向。
# ▌近距离裁剪半径，世界坐标单位。
# ▌0.0 = 关闭（不裁剪任何单元）。
...
out vec3  fNormal;
out float fFault;
out float fInterior;   // 兼容保留, 恒为 0
out float fAlpha;      // V25: 近端剥壳淡出系数 (0=全透明已隐藏, 1=不透明)
...
uniform vec3  uCamPos;          // V25: 相机世界坐标 (CPU 端已把 z 乘以 uVExag)
uniform float uProximityRadius; // V25: 剥壳半径 cullR (此半径内单元隐藏)
uniform float uShellRadius;     // V25: 兼容保留(未使用)
uniform float uTileScale;       // V25: 兼容保留(未使用)
...
vec3 vex(vec3 p) { return vec3(p.x, p.y, p.z * uVExag); }

// ============================================================
// V25: Proximity Culling — 靠近时近端单元让开, 看进油藏内部
// ============================================================
// 导师英文原话: "auto inactivation of cells that is close to camera,
//   creating the effect when observer enter in the field model"
// 草图: 眼睛从外指入模型 -> 看到内部结构。
//
// 机制(按单元中心到相机距离 dc):
//   dc < cullR              -> 隐藏(不发射)            => 看穿进内部
//   cullR <= dc < cullR+fade-> 平滑淡出(alpha 0->1)    => "不应只是简单消失"
//   dc >= cullR+fade        -> 正常显示(alpha=1)
// 淡出宽度 fade 取 cullR 的一半, 过渡自然。
// 近端让开后, 内部单元/井/断层正常显示 -> 观察者看进油藏。
float proximityAlpha(vec3 cc) {
    float cullR = max(uProximityRadius, 0.0);
    if (cullR <= 0.0) return 1.0;          // 半径=0 -> 关闭剥壳, 全部显示
    float dc   = distance(cc, uCamPos);
    float fade = cullR * 0.5;
    if (dc < cullR)        return 0.0;     // 隐藏
    if (dc < cullR + fade) return (dc - cullR) / fade;  // 淡出 0->1
    return 1.0;                            // 正常
}

void emitQuad(vec3 a, vec3 b, vec3 c, vec3 d, float depth, float fault, float alpha) {
    vec3 ae=vex(a), be=vex(b), ce=vex(c), de=vex(d);
    vec3 n = normalize(cross(be - ae, ce - ae));
    fNormal = n; fDepth = depth; fFault = fault; fInterior = 0.0; fAlpha = alpha;
...
void main() {
    ...
    // V25: 近端剥壳 — 隐藏或淡出
    vec3 cc = vec3(cx, cy, cz * uVExag);
    float alpha = proximityAlpha(cc);
    if (alpha <= 0.0) return;   // 完全隐藏: 直接不发射这个单元

    float fault = texelFetch(uFaultFlag, cell).r;

    // 6 个面（正常绕序; 恢复背面剔除, 不再双面）
    emitQuad(c0, c2, c1, c3, cz, fault, alpha);
    emitQuad(c4, c5, c6, c7, cz, fault, alpha);
    emitQuad(c0, c1, c4, c5, cz, fault, alpha);
    emitQuad(c3, c2, c7, c6, cz, fault, alpha);
    emitQuad(c2, c0, c6, c4, cz, fault, alpha);
    emitQuad(c1, c3, c5, c7, cz, fault, alpha);
}
"""
...
in  float fInterior;
// ▌V20: 内表面标志。0.0=外表面, 1.0=内表面。

in  float fAlpha;
// ▌V25: 近端剥壳淡出系数, 决定本片段透明度。
...
// ▌片段着色器入口: 对每个像素 (fragment) 执行一次。
    ...
    // V25: 近端剥壳 — 近乎全透明的片段直接丢弃, 其余按 fAlpha 做半透明淡出。
    if (fAlpha < 0.01) discard;
    FragColor = vec4(col * d, fAlpha);
    // ▌最终像素颜色 = 基础颜色 × 光照强度; alpha = 剥壳淡出系数。
}
"""
...
def main():
    ...
    def render_scene(mvp, width, height, cam_pos=None):
        ...
        # V25: 恢复背面剔除（剥壳是"去面", 不需要双面）;
        #      开启 alpha 混合以支持近端单元的平滑淡出。
        ...
        # V20修复: cam_pos.z 需要乘以 vexag，因为 shader 中
        # cellCenter.z = cz * uVExag，两者必须在同一坐标空间。
        if cam_pos is not None:
            glUniform3f(uCamPos, float(cam_pos[0]), float(cam_pos[1]), float(cam_pos[2]) * vexag)
        else:
            glUniform3f(uCamPos, 0.0, 0.0, 0.0)
        glUniform1f(uProxRad, float(proximity_radius))
        glUniform1f(uShellR, float(shell_radius))
        glUniform1f(uTile, float(tile_scale))
        ...
        glBindVertexArray(0)
        glDisable(GL_BLEND)  # V25: 油藏绘制完关闭混合
    ...
    # V20: 初始 shell/proximity 参数（VR 模式下每帧自动调整）
    global shell_radius
    shell_radius = sz * 0.15   # 弃用（占位）
    proximity_radius = 0.0     # V26: 由"到中心距离"自动驱动（见 VR 每帧逻辑）
    # V26: 自动进入的几何量
    half_x = float(p.max_b[0] - p.min_b[0]) * 0.5
    half_y = float(p.max_b[1] - p.min_b[1]) * 0.5
    model_radius = max(half_x, half_y) * model_radius_factor  # 触发进入的半径
    prox_max = sz * prox_max_factor                            # 中心处剥壳上限
    print(f"[V26] 自动进入: model_radius={model_radius:.0f}, prox_max={prox_max:.0f} "
          f"(走进模型自动剥壳, 无需按键; sz={sz:.0f})")
    ...
    uTile     = glGetUniformLocation(prog_res,  "uTileScale")
    ...
    # V29-fix: near 保持小值(确保模型能渲染, 不黑屏); 仅把 far 大幅拉小
    # 来改善 near/far 比例, 缓解 z-fighting 尖刺。
    # 旧: near=sz*0.001 / far=sz*50  -> near/far=2e-5  (尖刺)
    # 新: near=sz*0.001 / far=sz*3   -> near/far=3.3e-4 (精度提升~17倍, 且不黑屏)
    # far=sz*3≈45852m > 模型最远点(相机9000+半跨7642=16642m), 足够包含整个模型。
    near = sz * 0.001
    far  = sz * 3.0
    # ▌near/far = 0.001/3 ≈ 3.3e-4, 比旧的 2e-5 提升约17倍, 深度可区分性大幅改善。
    ...
    print("  I          : (desktop) enter / exit model interior")
    print("  (auto)     : Walk into the model -> interior reveals automatically")
    print("  N / B      : Tune auto reveal depth (prox_max) up / down")
    print("  P          : Toggle auto-enter on / off")
    print("  G          : (VR) recenter to model center")
    print("  R          : (VR) re-anchor orientation")
    print("  ESC        : Quit")
    print("================")
    ...
    prev_g = glfw.RELEASE  # V24: G 键边沿检测，用于一键回中心
    prev_p = glfw.RELEASE  # V26: P 键边沿检测，用于切换自动进入
    prev_t = glfw.RELEASE  # V29: T 键边沿检测，用于翻转前探方向
    # ▌记录 F、W、V 键的上一帧状态。
    ...
    while not glfw.window_should_close(win):
        ...
        # V23: 已移除气泡调参键（不再有顶点变形）。内部封闭由关闭背面剔除实现。
        ...
        # V24 新增: G 键 = 一键回到模型中心（演示时位置飘了, 按一下即回正中内部）
        # 实现: 把当前头显 pose 设为新原点 -> 相对位移归零 -> 回到中心(0,0,0)。
        cur_g = glfw.get_key(win, glfw.KEY_G)
        if cur_g == glfw.PRESS and prev_g == glfw.RELEASE:
            hmd_init_inv = None  # 下一帧重锚 -> 位置归零回中心
            print("[VR] G 键: 已请求回到模型中心, 下一帧生效")
        prev_g = cur_g

        # V26: 自动进入模式下, 剥壳半径由距离自动决定。
        #   N / B = 微调"中心处剥壳上限 prox_max"(让自动效果更深 / 更浅)
        #   P     = 切换自动进入开关(关掉后回到手动, N/B 直接调半径)
        prox_step = sz * 0.004
        if glfw.get_key(win, glfw.KEY_N) == glfw.PRESS:
            if auto_enter:
                prox_max = min(sz * 0.5, prox_max + prox_step)
            else:
                proximity_radius = min(sz * 1.0, proximity_radius + prox_step)
        if glfw.get_key(win, glfw.KEY_B) == glfw.PRESS:
            if auto_enter:
                prox_max = max(0.0, prox_max - prox_step)
            else:
                proximity_radius = max(0.0, proximity_radius - prox_step)
        cur_p = glfw.get_key(win, glfw.KEY_P)
        if cur_p == glfw.PRESS and prev_p == glfw.RELEASE:
            auto_enter = not auto_enter
            if not auto_enter:
                proximity_radius = 0.0
            print(f"[V26] 自动进入 = {auto_enter}")
        prev_p = cur_p

        # V29: T 键 = 翻转前探方向(若发现"头前探反而离模型更远", 按一下 T)
        cur_t = glfw.get_key(win, glfw.KEY_T)
        if cur_t == glfw.PRESS and prev_t == glfw.RELEASE:
            z_dir = -z_dir
            print(f"[V29] 前探方向已翻转, z_dir={z_dir:+.0f}")
        prev_t = cur_t
        ...
        #   位置缩放因子:   VR_SCALE_Z(前后) / VR_SCALE_XY(左右上下) - 分轴缩放
        #                  让用户用脚走几步就能从外部走进模型内部
        if vr_mode and vr_system is not None and vr_fbo_l is not None:
            try:
                ...
                if hmd_pose.bPoseIsValid:
                    ...
                    if hmd_init_inv is None:
                        ...
                        print(f"[VR]   ★ 你现在在模型外, 能看到油藏全貌 ★")
                        print(f"[VR]   头向前探即可钻进模型(进入后自动剥壳露出结构)")
                        print(f"[VR]   前后缩放×{VR_SCALE_Z:.0f}(头探即进), 左右上下×{VR_SCALE_XY:.0f}(转头不跟晃)")
                        print(f"[VR]   颈部模型补偿已启用, 转头模型不动")
                        print(f"[VR]   G 键=回模型中心   R 键=重新锚定朝向")
                    ...
                    # ════════════════════════════════════════════════════════
                    # V20: 颈部模型补偿 (Neck Model Compensation)
                    # ════════════════════════════════════════════════════════
                    # 问题: 头显追踪点在前额位置，不在脖子旋转中心。
                    #   点头/转头时追踪点画弧线，产生 5-10cm 的虚假位移。
                    #   乘以 VR_SCALE_XY 后变成虚拟跳动。
                    #
                    # 修复: 计算"纯旋转引起的追踪点弧线位移"并减掉它，
                    #   只保留真正的身体走路位移。
                    #
                    # V29: 颈部补偿只作用于 X/Y(横向), 不扣前后(Z)
                    # ════════════════════════════════════════════════════════
                    # 旧问题: 颈部补偿把"前探时头部不可避免的轻微俯仰"产生的
                    #   弧线位移当作旋转扣掉, 结果连真正的前探(Z)也被抵消 ->
                    #   头怎么探都进不去模型。
                    # 修复: 补偿只用于消除左右转头/上下点头的横向虚假位移(X/Y);
                    #   前后(Z)用原始位移, 保证前探一定能推进。
                    neck_pivot = np.array([0.0, -0.08, -0.10], dtype=np.float32)
                    R_rel = T_relative[:3, :3]
                    rot_only_pos = R_rel @ (-neck_pivot) + neck_pivot
                    pure_walk = hmd_pos_phys_rel - rot_only_pos  # X/Y 用补偿后

                    # V29: 分轴缩放 — 前后(Z)中等灵敏并保留完整前探, 左右上下小
                    #   Z 用"未被补偿扣减"的原始前后位移 -> 头前探一定能进入模型
                    #   Z 缩放 4000(原8000太灵敏, 一探冲过头) -> 前探~0.34m 进入
                    forward_phys = hmd_pos_phys_rel[2] * z_dir   # 原始前后位移 × 方向符号(T键可翻转)
                    hmd_pos_world = np.array([
                        pure_walk[0] * VR_SCALE_XY,
                        pure_walk[1] * VR_SCALE_XY,
                        forward_phys * VR_SCALE_Z
                    ], dtype=np.float32)
                    ...
                    if frame_count % 60 == 0:
                        ...
                        print(f"[VR追踪] 相对位移={hmd_pos_phys_rel[0]:+.2f},{hmd_pos_phys_rel[1]:+.2f},{hmd_pos_phys_rel[2]:+.2f}m "
                              ...
                              f"距离={dist:.0f} Prox={proximity_radius:.0f} Shell={shell_radius:.0f}")
                else:
                    # 头显姿态无效：使用安全的回退值
                    # V27: 起点在模型外(0,0,9000), 看向模型中心(原点)
                    _fb = np.array([0.0, 0.0, VR_USER_OFFSET], dtype=np.float32)
                    V_vr = mat_lookat(_fb,
                ...
                # ════════════════════════════════════════════════════════════
                # V26: 自动进入 — 按"相机到模型中心距离"自动设定剥壳半径
                #   dc >= model_radius (在外面)  -> 0, 模型完整
                #   dc <  model_radius (进入)    -> 越深入剥壳越大
                #   dc -> 0 (中心)               -> 上限 prox_max
                # 纯靠走动触发, 无需按键; 退出自动归零。
                if auto_enter:
                    dc_center = float(np.linalg.norm(vr_cam_pos))
                    if dc_center >= model_radius:
                        proximity_radius = 0.0
                    else:
                        # 深入比例 0(边缘)->1(中心); 平滑(smoothstep)
                        u = 1.0 - dc_center / max(model_radius, 1.0)
                        u = max(0.0, min(1.0, u))
                        u = u * u * (3.0 - 2.0 * u)  # smoothstep, 进入更柔和
                        proximity_radius = prox_max * u
                shell_radius = sz * 0.15  # 未使用（占位）
            ...
            # -------- 普通窗口模式渲染 --------
            glBindFramebuffer(GL_FRAMEBUFFER, 0)
            # V26: 桌面也自动进入 — 按相机到中心距离设剥壳半径
            if auto_enter:
                dc_center = float(np.linalg.norm(eye))
                if dc_center >= model_radius:
                    proximity_radius = 0.0
                else:
                    u = 1.0 - dc_center / max(model_radius, 1.0)
        ...
        glfw.set_window_title(win,
            f"Norne v29 [{mode_str}]  |  X:{slice_x*100:.0f}%  Z:{slice_z*100:.0f}%  "
            f"VExag:{vexag:.1f}x  "
            f"Faults:{'ON' if show_faults else 'OFF'}  "
            f"Wells:{'ON' if show_wells else 'OFF'}  "
            f"Prox:{proximity_radius:.0f}  Auto:{'ON' if auto_enter else 'OFF'}  " + ("[VR: 头前探进入(自动剥壳)  T=方向反了按它  G=回起点  R=重锚]" if vr_mode else f"[I]={'退出内部' if inside_mode else '进入内部'}(自动剥壳)  P=自动开关 [V]=VR"))
